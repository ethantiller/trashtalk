...
export async function POST(request) {
  ...
  IMPORTANT PRIORITIZATION FOR ITEM IDENTIFICATION:
  - If the System text (from model) is exactly "other" (case-insensitive), then you should prioritize your own analysis (Gemini's capabilities) of the image and context as the most reliable source for what the item is. Only use the user description if both the System text and your own analysis are inconclusive.
  - If the System text is anything other than "other", then the System text (huggingfaceText) is the most important and reliable information about the item and should be treated as the primary source for what the item is.
  - If there is any conflict or ambiguity, follow this order of priority:
    1. System text (huggingfaceText), unless it is "other"
    2. Gemini's own analysis (image/context)
    ...
    CONFLICT RESOLUTION BETWEEN SOURCES (VERY IMPORTANT):
    - If the System text (huggingfaceText) and your own analysis (Gemini) or the user description disagree, always treat the System text as the most reliable, unless it is clearly missing or nonsensical.
    - If the System text is missing or unclear, use your own analysis (Gemini) as the next best source.
    - Only use the user description if both the System text and your own analysis are inconclusive.
    - If the image and text sources clearly disagree about the basic nature of the item, use the above priority order: System text > Gemini analysis > user description.
    - If the text makes an extreme, implausible, or unrealistic claim that is not clearly supported by the System text or image, assume the claim is incorrect, exaggerated, or a joke and IGNORE it when determining disposal category, legal requirements, and redemption value.
    - Do NOT invent exotic or specialized disposal scenarios (such as precious metal refining, hazardous waste handling, or very high-value scrap) unless both the System text AND the image support that conclusion clearly and it is realistic for a normal household.
    - When you ignore an implausible claim, still provide normal, sensible guidance for what the item appears to be in the System text or image (for example, "clothing" instead of "pure gold clothing").
